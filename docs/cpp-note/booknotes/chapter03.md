...
在我的 Windows 10 64位 OS 上，数据类型是这样的: 
...
```c++
//测试代码
unsigned char un = UCHAR_MAX;//0xff
char ch = CHAR_MAX;
signed char signedCh = SCHAR_MAX;//127
printf("charMax= %x ,scharMax= %x \n", ch, signedCh);//0x7f ,0xs7f
```
...
参考网站：https://www.cnblogs.com/qingergege/p/7507533.html
